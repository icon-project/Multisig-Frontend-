...
export const injectiveExecuteMemberManagement = async (
  chainId: string,
  ...
  try {
    const res = await executeInjectiveContractCall(chainId, contractAddress, proposeMsg);
    if (res) {
      return res;
    }
  } catch (err) {
...
export const injectiveExecuteContractUpgrade = async (
  chainId: string,
  contractAddress: string,
  proposalTitle: string,
  proposalDescription: string,
